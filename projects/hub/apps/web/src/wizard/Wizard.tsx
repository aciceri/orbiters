          ...
          <dl className="divide-y rounded-2xl border bg-card">
            {steps.map((candidate, at) => (
              ...
              >
                ...
                {/* The "Modifica" control lives inside the dd, as a second flex child next
                    to the summary text, rather than as a third sibling of dt/dd: axe's
                    definition-list rule flattens this row's wrapping div and rejects a
                    button found among its direct children (a div > button is not a valid
                    dt/dd group), but it never looks inside a dd itself. Nesting it here
                    keeps the row spacing and wrap identical (REB-96). */}

...
revoked token and a deactivated owner are not told apart, so a leaked token is not an
oracle for whether it is still worth using.
"""
...
from rebase_core.models import NAME_MAX_LENGTH, AdminToken, AdminUser
...
class AdminTokenService:
    ...
    def create(self, admin_id: UUID, nome: str) -> tuple[AdminTokenRead, str]:
        ...
        if admin is None or not admin.attivo:
            raise NotFound("admin", admin_id)
    ...
    def revoke(self, admin_id: UUID, token_id: UUID) -> AdminTokenRead:
        ...
        row = self.session.scalar(
            select(AdminToken).where(AdminToken.id == token_id, AdminToken.admin_id == admin_id)
        )
        if row is None:
    ...
    def resolve(self, raw: str | None) -> AdminRead:
        ...
        `last_used_at`, which is what the list shows as «ultimo uso»."""
        if not is_token(raw):
        ...
        if row is None or row.revoked_at is not None:
            raise ValidationFailed(ENTITY, "token", INVALID_TOKEN)
        admin = self.session.get(AdminUser, row.admin_id)
        if admin is None or not admin.attivo:
            raise ValidationFailed(ENTITY, "token", INVALID_TOKEN)
    ...
    def list(self, admin_id: UUID) -> list[AdminTokenRead]:
        ...
        rows = self.session.scalars(
            select(AdminToken)
            .where(AdminToken.admin_id == admin_id)
